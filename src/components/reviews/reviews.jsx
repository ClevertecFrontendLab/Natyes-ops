...
export const Reviews = (props) => {
    const reviews = useSelector(state => state.library.currentBook.comments);
    const cls = reviews ? 'title' : 'title reviews-no';
    ...
    return ( 
        <div className="reviews">
            <h5 className={cls}>
                {props.title} 
                <span className="count body-s">{reviews ? reviews.length : ''}</span>
            ...
            <div className={`reviews-list ${toggleReview()}`}>
                {reviews && reviews.map(el => <Review {...el} key={el.id}/>)}
            </div>
